...
#-----------------------------------------------------------------------------
# Date: January 16, 2019 (last updated: November 11, 2022)
# Authors: Clément Cabriel [1], Jean Commère [1], Caroline Schou [2]
# Affiliations: [1] Institut des Sciences Moléculaires d'Orsay, CNRS UMR8214, Université Paris-Sud, France; [2] Abbelight, Paris, France.
# Corresponding author: Clément Cabriel (clement.cabriel@espci.fr, cabriel.clement@gmail.com)
#-----------------------------------------------------------------------------
# DAISY correction program.
# This code is provided as supporting material for the manuscript [Cabriel et al., 'Combining 3D single molecule localization strategies for reproducible bioimaging', Nature Communications 10, 1980 (2019); doi: https://doi.org/10.1038/s41467-019-09901-8 ]. Please refer to the manuscript and to the Readme file for more details about the content and the use of the code.
...
#-----------------------------------------------------------------------------
# INPUT FORMAT
# Localization .txt file (column-wise): y, x, z_SAF, z_astigmatism, frame number (all values in nm except the number of the image)
...
#-----------------------------------------------------------------------------
# OUTPUT FORMATS
# Corrected localization '..._DAISYcorrected.txt' file (column-wise): y, x, z_SAF, z_astigmatism, frame number (all values in nm except the number of the image)
# 'Tilt.txt' file: contains the optimal fitted plane for the tilt and offset corrections
# 'DirftZ.txt' file: contains the axial drift results (column-wise): frame number, z value (nm)
#-----------------------------------------------------------------------------
...
#-----------------------------------------------------------------------------
        ...
        # Input Files Data
        self.filepath = u"DAISY_data_test_COS7-alphatubulin-AF647_1.txt"      # path to the input localization file
...
# The following functions are kindly provided by the software team of Abbelight. These functions were developed following a common discussion on the project between the authors.
    ...
    def AxialShiftCorrect(self, coords1, coords2, imsizeX, imsizeY, zrange, pixelsize, binsize_lateral, binsize_axial, plot):
        ...
        autocorrfunc = []
        for i in range(-int(imsizeZ/2), int(imsizeZ/2)+1) :
            Tz = i
